...
/*
 * CUDA version of the 2D Lennard-Jones simulation.
 *
 * The main idea is:
 *   1. Keep particle data on the GPU during the whole simulation.
 *   2. Store particle fields in separate arrays: x[], y[], vx[], vy[], fx[], fy[].
 *      This is called SoA (structure of arrays) and is friendlier to GPU memory access
 *      than repeatedly reading/writing an array of Particle structs.
 *   3. Parallelise the expensive force computation. One CUDA block computes the
 *      total force for one particle. Threads inside that block split the loop over
 *      all other particles and then reduce partial sums in shared memory.
 *   4. Copy data back to the CPU only at the end, except when optional energy
 *      logging or GIF generation requires intermediate values.
 */

#ifndef FORCE_THREADS
// Number of threads per CUDA block for force computation.
// One block computes one particle, so this controls how many threads split
// the inner j-loop for that particle.
#define FORCE_THREADS 256
#endif

#ifndef REDUCE_THREADS
// Number of threads per block for vector-style kernels and reductions.
// It does not have to be equal to FORCE_THREADS; it is chosen separately.
#define REDUCE_THREADS 256
#endif

// Small helper macro: after every CUDA API call we immediately check whether
// an error occurred. This makes debugging much easier, because the program
// fails at the line where the CUDA error first appears.
...
#if GENERATE_GIF
// Optional visualisation code. This is useful for checking that the simulation
// looks reasonable, but it should be disabled for timing measurements because
// it copies particles back to the CPU and writes frames.
...
// Uniform random value in [0, 1]. Used only during CPU initialisation.
...
/*
 * Initialise particles on the CPU.
 *
 * Particles are placed on a regular 2D grid with small random jitter.
 * Velocities are random, then shifted so that average velocity is zero
 * and scaled to match the requested temperature.
 *
 * This part is not performance-critical for the assignment, because it runs
 * once before the timed simulation loop.
 */
...
// CPU version of periodic boundary wrapping. Kept for the CPU fallback code.
...
// Shift of the Lennard-Jones potential at the cutoff radius.
// Marked as both __host__ and __device__, so the same formula can be used
// from CPU code and CUDA kernels.
 ...
 *
 * This is not used by the CUDA path inside run_simulation, but is useful for
 * correctness checks, comparison against the original implementation, and for
 * keeping compatibility with the original code structure.
 */
...
/*
 * CPU fallback for one Leapfrog step.
 *
 * CUDA run_simulation implements the same three logical phases with kernels:
 *   1. half velocity update + position update,
 *   2. force recomputation,
 *   3. second half velocity update.
 */
 ...
 *
 * CPU/main code uses Particle structs, but CUDA kernels use separate arrays for
 * better memory access patterns and simpler coalesced reads/writes.
 */
...
/*
 * Convert separate GPU arrays back into the Particle struct array.
 *
 * This is called at the end of run_simulation, and optionally during GIF
 * generation, so the caller receives the final particle state in the original
 * format expected by SimulationResult/main.c.
 */
...
/*
 * First part of the Leapfrog integration step.
 *
 * For each particle, one CUDA thread:
 *   - updates velocity by half a time step using the previous force,
 *   - updates position using the half-step velocity,
...
/*
 * Second part of the Leapfrog integration step.
 *
 * After the new forces have been computed, each thread finishes the velocity
 * update for one particle.
 */
...
/*
 * Main CUDA force kernel.
 *
 * Work division:
 *   - gridDim.x == n, so each CUDA block is responsible for one particle i;
 *   - threadIdx.x inside the block splits the loop over all particles j;
 *   - each thread accumulates a partial Fx, Fy and potential energy;
 *   - partial sums are reduced in shared memory;
 *   - thread 0 writes the final force for particle i to global memory.
 *
 * This avoids an expensive global N x N force matrix. The pair interaction is
 * computed twice (i->j and j->i), but there are no atomics and no write conflicts.
 */
...
void compute_forces_kernel(const double *__restrict__ x,
    ...
    //   sfx[0..blockDim.x-1]  - partial x-force sums
    //   sfy[0..blockDim.x-1]  - partial y-force sums
    //   spe[0..blockDim.x-1]  - partial potential energy sums
    ...
    // Split the inner j-loop between threads in this block.
    // Example: with 128 threads, thread 0 checks j=0,128,256,...;
    // thread 1 checks j=1,129,257,... and so on.
    for (unsigned int j = tid; j < n; j += blockDim.x) {
        ...
        // use the closest periodic image of particle j relative to particle i.
    ...
    // Store per-thread partial sums into shared memory before reduction.
    sfx[tid] = local_fx;
    sfy[tid] = local_fy;
    spe[tid] = local_pe;
    __syncthreads();
    ...
    // 128 values -> 64 -> 32 -> ... -> 1 final value.
...
// Compute one kinetic-energy term per particle. A separate reduction kernel then
// sums these terms into the total kinetic energy.
...
/*
 * Generic parallel reduction kernel for summing an array of doubles.
 *
 * Each block reads up to 2 * blockDim.x elements, reduces them in shared memory,
 * and writes one partial sum to out[blockIdx.x]. reduce_device_sum calls this
...
/*
 * Sum an array that already lives on the GPU and return the scalar to the CPU.
 *
 * This performs several GPU reduction passes and then a final cudaMemcpy of one
 * double from device to host. Therefore, when called inside the timed region,
 * the scalar transfer is included in the benchmark time.
 */
static double reduce_device_sum(const double *d_values,
    ...
    // Repeatedly reduce partial sums until only one value remains.
    while (cur_n > 1) {
...
// Thin wrapper around the main force kernel. It computes the required shared
// memory size and checks for kernel launch errors.
...
// Kinetic energy on the GPU: first compute one term per particle, then reduce
// all terms into one scalar.
...
/*
 * CUDA implementation of the full simulation.
 *
 * Important for benchmarking:
 *   - main.c measures time around the whole run_simulation call;
 *   - the initial cudaMemcpy Host->Device and final cudaMemcpy Device->Host are
 *     inside this function, so transfer times are included;
 *   - intermediate CPU transfers are avoided unless log_steps or GIF generation
 *     is enabled.
 */
SimulationResult run_simulation(Particle *particles,
    ...
    // Memory sizes and launch configuration for vector-style kernels.
    ...
    // Device pointers. d_particles is used only for input/output compatibility.
    // The actual simulation works on the SoA arrays below.
    ...
    // Allocate all device memory once before the simulation loop.
    ...
    // Initial CPU -> GPU transfer. Because this is inside run_simulation, it is
    // included in the measured runtime from main.c.
    CUDA_CHECK(cudaMemcpy(d_particles, particles, (size_t)n * sizeof(Particle), cudaMemcpyHostToDevice));
    // Convert Particle structs into separate GPU arrays.
    pack_particles_kernel<<<vector_blocks, REDUCE_THREADS>>>(d_particles, d_x, d_y, d_vx, d_vy, d_fx, d_fy, n);
    CUDA_CHECK(cudaGetLastError());

    // Compute initial forces and initial energies before the first step.
    ...
    if (!gif) {
        ...
        // Final GPU -> CPU transfer. This is also inside run_simulation, so it is
    // included in the measured runtime.
    ...
    // Main time-stepping loop. The loop itself is sequential because step k+1
    // depends on the result of step k, but the work inside each step is parallel.
    for (unsigned int step = 0; step < nsteps; ++step) {
        // Leapfrog phase 1: half velocity update + position update + wrapping.
        integrate_first_kernel<<<vector_blocks, REDUCE_THREADS>>>(d_x, d_y, d_vx, d_vy, d_fx, d_fy, n, box_size);
        CUDA_CHECK(cudaGetLastError());

        // Recompute forces at the new positions. This is the dominant O(N^2) work.
        compute_forces_gpu(d_x, d_y, d_fx, d_fy, d_pe_rows, n, box_size);

        // Leapfrog phase 2: finish the velocity update using the new force.
        integrate_second_kernel<<<vector_blocks, REDUCE_THREADS>>>(d_vx, d_vy, d_fx, d_fy, n);
        CUDA_CHECK(cudaGetLastError());

        // Optional energy tracking. Disabled during benchmark runs because it
        // performs reductions and scalar Device->Host copies every step.
        if (log_steps) {
        ...
        if (gif && FRAME_EVERY > 0 && (step + 1) % FRAME_EVERY == 0) {
            // Convert SoA arrays back to Particle structs and copy final state to CPU.
    ...
    // If per-step logging was disabled, compute final energy only once.
    ...
    // Free all device allocations before returning to main.c.
